...
internal static class StringExtensions
{
    ...
    internal static string EncodePath(this string path) => EncodeRegex.Replace(path, m => EscapeChar + ((short)m.Value[0]).ToString("X4"));

    internal static string DecodePath(this string path) => DecodeRegex.Replace(path, m => ((char)Convert.ToInt16(m.Groups[1].Value, 16)).ToString());
}
